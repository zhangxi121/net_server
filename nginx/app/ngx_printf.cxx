...
//------------------------------------------------------------------------------------
// 对于 nginx 自定义的数据结构进行标准格式化输出,就像 printf,vprintf 一样，学习写这类函数到底内部是怎么实现的
...
// buf：往这里放数据, 写数据的起始位置,
// last：放的数据不要超过这里, 写数据的末尾位置, 防止越界,
// fmt：以这个为首的一系列可变参数,  支持的格式: %d【%Xd/%xd】:数字,    %s:字符串      %f：浮点,  %P：pid_t
// args: 可变参数,
// 对于：ngx_log_stderr(0, "invalid option: \"%s\",%d", "testinfo",123);
// fmt = "invalid option: \"%s\",%d"
// args = "testinfo", 123, ...  可变参数,
