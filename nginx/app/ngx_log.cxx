...
//-------------------------------------
// 描述: 通过可变参数组合出字符串(支持...省略号形参), 自动往字符串最末尾增加换行符(所以不用'\n'),
//      如果err不为0, 表示有错误, 会将该错误编号以及对象的错误信息一并放入到组合出的字符串中一起显示,
//
// va_start,va_end,
...
/*
    ...
    ngx_log_stderr(0, "invalid option: %010d", 21);        // nginx: invalid option: 0000000021  ---21前面有8个0补位,
...
{
    va_list args;
    u_char errstr[NGX_MAX_ERROR_STR + 1] = {0}; // 2048  -- ************  +1是我自己填的，感觉官方写法有点小瑕疵，所以动手调整一下
    u_char *p, *last;
    ...
    p = ngx_cpymem(errstr, "nginx: ", 7); // p指向"nginx: "之后的位置, 后面有个 + (n) 这个, 使指向拷贝后的下一个位置,

    va_start(args, fmt);                   // 使args指向起始的参数 fmt, 然后才是其他的扩展参数,
