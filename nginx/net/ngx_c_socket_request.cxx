...
// 来数据时候的处理，当连接上有数据来的时候，本函数会被ngx_epoll_process_events()所调用  ,官方的类似函数为 ngx_http_wait_request_handler();
void CSocekt::ngx_read_request_handler(lpngx_connection_t pConn)
{
    ...
    else if (pConn->curStat == _PKG_BD_INIT)
    {
        ...
        {
            // 收到的宽度小于要收的宽度, 因为在 ngx_wait_request_handler_proc_p1() 已经处理了当接受完包头时候,   "c->precvbuf = pTmpBuffer + m_iLenPkgHeader;  c->irecvlen = e_pkgLen - m_iLenPkgHeader; ", 此时 c->precvbuf  c->irecvlen 已经代表包体的部分,
...
ssize_t CSocekt::recvproc(lpngx_connection_t c, char *buff, ssize_t buflen) // ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    if (n < 0) // 这被认为有错误发生
    {
        // EAGAIN 和 EWOULDBLOCK [【这个应该常用在hp上】应该是一样的值，表示没收到数据，一般来讲，在ET模式下会出现这个错误，因为ET模式下是不停的recv肯定有一个时刻收到这个errno，但LT模式下一般是来事件才收，所以不该出现这个返回值
        ...
        if (errno == ECONNRESET) // #define ECONNRESET 104 /* Connection reset by peer */ ,对方很粗暴无礼的不关闭 socket, 直接把应用程序关闭了, 没有四次挥手而是RST复位包,
        {
            ...
            // do nothing
            // ngx_log_stderr(errno, "222222222222222222222222."); // 客户端直接粗暴地杀死进程, 服务器收到这个, (104: Connection reset by peer) ,
        ...
        // ngx_log_stderr(0, "连接被客户端 非 正常关闭！"); // 客户端直接粗暴地杀死进程, 走到这里,
...
void CSocekt::ngx_wait_request_handler_proc_p1(lpngx_connection_t pConn, bool &isflood)
{
    ...
    LPCOMM_PKG_HEADER pPkgHeader;                        // 这是一个指针,
    pPkgHeader = (LPCOMM_PKG_HEADER)pConn->dataHeadInfo; //正好收到包头时，包头信息肯定是在dataHeadInfo里；

    unsigned short e_pkgLen;
    e_pkgLen = ntohs(pPkgHeader->pkgLen); // 注意这里网络序转本机序，所有传输到网络上的2字节数据，都要用 htons() 转成网络序，所有从网络上收到的2字节数据，都要用 ntohs() 转成本机序,
...
ssize_t CSocekt::sendproc(lpngx_connection_t c, char *buff, ssize_t size) // ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    for (;;)
    {
        ...
        if (n == 0)
        {
            ...
            //遵循一个原则，连接断开，并不在send动作里处理，集中到recv那里处理，否则send,recv都处理都处理连接断开会乱套
        ...
        {
            // 内核缓冲区满，这个不算错误, 该错误产生的原因是由于send 函数中的size变量大小超过了tcp_sendspace的值。tcp_sendspace定义了应用在调用send之前能够在kernel中缓存的数据量。
            return -1; // 表示发送缓冲区满了,
        }
