...
ngx_signal_t signals[] = {
    ...
    {SIGSYS, "SIGSYS, SIG_IGN", NULL},        // 想忽略这个信号，SIGSYS表示收到了一个无效系统调用，如果不忽略，进程会被操作系统杀死，--标识31
                                              // 所以把handler设置为NULL，代表 我要求忽略这个信号，请求操作系统不要执行缺省的该信号处理动作（杀掉我）
    //...日后根据需要再继续增加
...
static void ngx_signal_handler(int signo, siginfo_t *siginfo, void *ucontext)
{
    ...
    if (ngx_process == NGX_PROCESS_MASTER) // master进程，管理进程，处理的信号一般会比较多
    {
        ...
        switch (signo)
        {
        case SIGCHLD:     // 一般子进程退出, 会收到该信号
            ngx_reap = 1; // 标记子进程状态变化，日后master主进程的for(;;)循环中可能会用到这个变量【比如重新产生一个子进程】
            break;

        case SIGUSR1: // 子进程创建成功, 手动给父进程发送一个 kill(getppid(), SIGUSR1),
            ngx_log_stderr(0, "=== sigaction SIGUSR1===");
            break;
            
            static int child_num = 0;
            child_num++;
            if (child_num == g_workprocess)
            {
    ...
    // 子进程状态有变化，通常是意外退出【既然官方是在这里处理，也学习官方在这里处理】
    if (signo == SIGCHLD)
    {
        // 一般子进程退出, 会收到 SIGCHLD,
...
static void ngx_process_get_status(void)
{
    ...
    for (;;)
    {
        // 这里用 for() 让 waitpid() 返回两次, 第一次返回子进程 pid, 第二次由于没有等到子进程, 所以返回0, 这样让 for() 退出,
        // $ ps -eo pid,ppid,sid,tty,pgrp,comm,stat,cmd | grep -E 'bash|PID|nginx'
        //  ---- 19747      1  19747 ?         19747 nginx           Ss   master process ./nginx
        //  ---- 19748  19747  19747 ?         19747 nginx           S    worker process
        //  ---- 19749  19747  19747 ?         19747 nginx           S    worker process
        //  ---- 19750  19747  19747 ?         19747 nginx           S    worker process
        //  ---- 19751  19747  19747 ?         19747 nginx           S    worker process
        // $ kill -9  19750
        // 2022/08/03 09:03:26 [notice] 19747: signal 17 (SIGCHLD) received from 19750
        // 2022/08/03 09:03:26 [alert] 19747: pid = 19750 exited on signal 9!
        //
